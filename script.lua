...
local PREMIUM_KEY_FILE = "MichelPremiumKey_V888.txt"
local PREMIUM_RAW_URL = "https://pastefy.app/63zSf0de/raw"
local EXPIRE_TIME = 7 * 3600 -- 7 Hours

-- Storage Functions
local function SaveKey(key, filename)
    if writefile then
        pcall(function()
            local data = { key = key, time = os.time() }
            writefile(filename or KEY_FILE, HttpService:JSONEncode(data))
        end)
    end
end

-- FIX 1: Memperbaiki JSONDecode (sebelumnya JSONEncode) agar data ter-load dengan benar
local function GetSavedKeyData(filename)
    local targetFile = filename or KEY_FILE
    if isfile and readfile and isfile(targetFile) then
...
local function IsKeyValid(keyData)
    ...
    local isMichelKey = string.sub(key, 1, 7) == "MICHEL-" and #key >= 15
    local isNotExpired = (os.time() - timestamp) < EXPIRE_TIME or string.find(key, "LIFETIME")
    
    return isMichelKey and isNotExpired
end
...
local function LoadPremiumScript()
    task.wait(0.5)
    loadstring(game:HttpGet(PREMIUM_RAW_URL))()
end

-- Premium Key Online Verification Function
local function VerifyPremiumKeyOnline(inputKey)
    local success, response = pcall(function()
        return game:HttpGet(PREMIUM_RAW_URL)
    end)
    
    if success and response then
        -- Cek jika input key cocok dengan data di Pastefy
        if string.find(response, inputKey, 1, true) or string.find(inputKey, "LIFETIME") then
            return true
        end
    end
    return false
...
-- 1. AUTO LOAD PREMIUM (FIXED)
local savedPremiumData = GetSavedKeyData(PREMIUM_KEY_FILE)
if savedPremiumData and savedPremiumData.key then
    if VerifyPremiumKeyOnline(savedPremiumData.key) then
        LoadPremiumScript()
        return
    else
        if delfile then pcall(delfile, PREMIUM_KEY_FILE) end
    end
end

-- 2. AUTO LOAD STANDARD KEY
local savedData = GetSavedKeyData(KEY_FILE)
...
local COL_GOLD        = Color3.fromRGB(251, 191, 36)
...
local function ShowNotification(message, kind)
    ...
    elseif kind == "success" then color = COL_SUCCESS
    elseif kind == "premium" then color = COL_GOLD end
...
ShadowHolder.Size = UDim2.new(0, 420, 0, 310)
ShadowHolder.Position = UDim2.new(0.5, -210, 0.5, -155)
...
InputLabel.Position = UDim2.new(0, 16, 0, 74)
...
-- State Flag Premium
local isPremiumMode = false

-- Buttons
local ApplyBtn = Instance.new("TextButton", MainFrame)
ApplyBtn.Size = UDim2.new(1, -32, 0, 40)
ApplyBtn.Position = UDim2.new(0, 16, 0, 146)
...
local PremiumBtn = Instance.new("TextButton", MainFrame)
PremiumBtn.Size = UDim2.new(1, -32, 0, 36)
PremiumBtn.Position = UDim2.new(0, 16, 0, 194)
PremiumBtn.BackgroundColor3 = COL_GOLD
PremiumBtn.BackgroundTransparency = 0.8
PremiumBtn.Text = "PREMIUM MODE"
PremiumBtn.TextColor3 = COL_GOLD
PremiumBtn.Font = Enum.Font.GothamBold
PremiumBtn.TextSize = 13
PremiumBtn.AutoButtonColor = false
PremiumBtn.SelectionImageObject = DummySelection
PremiumBtn.ZIndex = 3
corner(PremiumBtn, 9)
local PremStroke = stroke(PremiumBtn, COL_GOLD, 1, 0.5)

local GetKeyBtn = Instance.new("TextButton", MainFrame)
GetKeyBtn.Size = UDim2.new(1, -32, 0, 36)
GetKeyBtn.Position = UDim2.new(0, 16, 0, 238)
...
local function ToggleMode(toPremium)
    isPremiumMode = toPremium
    KeyInput.Text = ""
    
    if isPremiumMode then
        InputLabel.Text = "PREMIUM CODE"
        InputLabel.TextColor3 = COL_GOLD
        KeyInput.PlaceholderText = "Enter Premium Code..."
        ApplyBtn.BackgroundColor3 = COL_GOLD
        ApplyBtn.Text = "Submit Premium Code"
        PremiumBtn.Text = "STANDARD MODE"
        PremiumBtn.TextColor3 = COL_SKY
        PremStroke.Color = COL_SKY
    else
        InputLabel.Text = "LICENSE KEY"
        InputLabel.TextColor3 = COL_SKY
        KeyInput.PlaceholderText = "MICHEL-XXXXXXXX"
        ...
        PremiumBtn.Text = "PREMIUM MODE"
        PremiumBtn.TextColor3 = COL_GOLD
        PremStroke.Color = COL_GOLD
    end
end

PremiumBtn.MouseButton1Click:Connect(function()
    ToggleMode(not isPremiumMode)
end)
...
corner(ToggleBtn, 14)
stroke(ToggleBtn, COL_GLASS_STROKE, 1.2, 0.6)
...
ApplyBtn.MouseButton1Click:Connect(function()
    ...
    if isPremiumMode then
        -- Mode Premium
        ShowNotification("Verifying Premium Code...", "premium")
        if VerifyPremiumKeyOnline(key) then
            SaveKey(key, PREMIUM_KEY_FILE)
            TweenService:Create(InputStroke, TweenInfo.new(0.15), {Color = COL_GOLD, Transparency = 0.2}):Play()
            ShowNotification("Premium Verified! Loading Script...", "success")
            task.wait(1)
            ScreenGui:Destroy()
            LoadPremiumScript()
        else
            TweenService:Create(InputStroke, TweenInfo.new(0.15), {Color = COL_ERROR, Transparency = 0.2}):Play()
            ShowNotification("Invalid or Expired Premium Code!", "error")
        end
    else
        -- Standard Mode / Key Lifetime dari Web
        if string.sub(key, 1, 7) == "MICHEL-" and #key >= 15 then
            if string.find(key, "LIFETIME") then
                -- Jika memasukkan key Lifetime di mode Standard, langsung simpan sebagai Premium
                SaveKey(key, PREMIUM_KEY_FILE)
            else
