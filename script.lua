...
local function GetSavedKeyData(filename)
    ...
    if isfile and readfile and isfile(targetFile) then
        local success, result = pcall(function()
            return HttpService:JSONEncode(readfile(targetFile))
        end)
...
-- FIX: Validasi key dibuat fleksibel (tidak lagi memaksa 15 karakter)
...
local function VerifyPremiumKeyOnline(inputKey)
    ...
    if success and response then
        if string.find(response, inputKey, 1, true) then
            return true
        end
    end
    return false
...
ApplyBtn.MouseButton1Click:Connect(function()
    ...
    if isPremiumMode then
        ...
        -- FIX: Mode Key Standard sekarang menerima key minimal 15 karakter (seperti MICHEL-LIFETIME-...)
