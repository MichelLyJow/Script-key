...
-- 1. AUTO LOAD PREMIUM (Pastefy Script)
...
-- 2. AUTO LOAD STANDARD KEY (GitHub Script)
local savedData = GetSavedKeyData(KEY_FILE)
if IsKeyValid(savedData) then
    if string.find(savedData.key, "LIFETIME") then
        LoadPremiumScript()
    else
        LoadMainScript()
    end
...
ApplyBtn.MouseButton1Click:Connect(function()
    ...
    if isPremiumMode then
        ...
        if string.sub(key, 1, 7) == "MICHEL-" and #key >= 15 then
            local isLifetime = string.find(key, "LIFETIME")
            ...
            -- PERBAIKAN DI SINI: Jika Key mengandung "LIFETIME", wajib panggil LoadPremiumScript()
            if isLifetime then
                LoadPremiumScript()
            else
                LoadMainScript()
